...
public class MessageController {
    ...
    @PostMapping("/message")
    public String postMessage(@ModelAttribute Message message, Model model, @AuthenticationPrincipal OAuth2User user) {
        message.setUser(user.getAttribute("name"));
        repository.save(message);
        model.addAttribute("message", repository.findAll());
